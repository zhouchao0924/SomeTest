...
void FUE4EditorCustomizeModule::StartupModule()
{
	...
	//https://blog.ch-wind.com/ue4-config-setting/
	//https://blog.ch-wind.com/ue4-config-usage/
	//https://docs.unrealengine.com/zh-CN/Programming/Basics/ConfigurationFiles/index.html
	...
	FModuleManager::LoadModuleChecked<FPropertyEditorModule>("PropertyEditor").RegisterCustomClassLayout(
		UEditorCustomizeSetting::StaticClass()->GetFName(),
		FOnGetDetailCustomizationInstance::CreateLambda([]() 
	{
		return MakeShareable(new UE4ECSettingsCustomization); 
	}));
	...
	SettingS = FModuleManager::LoadModuleChecked<ISettingsModule>("Settings").RegisterSettings(
		"Project", 
		"Plugins",
		TEXT("UE4EditorCustomize"), 
		FText::FromString("UE4 EditorCustomize"),
		FText::FromString("Setting For UE4EditorCustomize"), 
		GetMutableDefault<UEditorCustomizeSetting>());
...
#undef APPLY_STYLE
	...
	tmpFunc(CoreStyles, StyleSettings->CustomStyleCore);
}
